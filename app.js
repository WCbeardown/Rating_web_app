...
async function loadData() {
  try {
    ...
    const response = await fetch("data/rating_data_all.csv", { cache: "no-cache" });
    ...
    allData = parseCSV(text)
      .map(row => ({
        ...
        member: Number(row["会員番号"]),
        rating: Number(row["レイティング"])
      }))
      .filter(row => row.date && Number.isFinite(row.member) && Number.isFinite(row.rating))
      .sort((a, b) => a.date - b.date);

    if (!allData.length) throw new Error("有効なデータがありません。");
    ...
    setStatus(`データ読み込み完了：${allData.length.toLocaleString("ja-JP")}件`, "ok");
    renderApp(false);
  } catch (error) {
    console.error(error);
...
function renderDetailTables(selected) {
  ...
  selected.forEach((person, index) => {
    ...
    title.textContent = `${index + 1}人目：${person.member}`;
